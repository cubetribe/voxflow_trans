      ...
      <div className="space-y-3">
        ...
        {file.status === 'completed' && file.result && (
          <div className="mt-4 p-3 bg-gray-800/50 rounded-lg">
            <div className="text-xs text-gray-400 mb-1">Transcription Result:</div>
            <div className="text-sm text-gray-200 max-h-32 overflow-y-auto">
              {file.result.text || (file.result.segments && file.result.segments.map((s: any) => s.text).join(' ')) || 'No text available'}
            </div>
          </div>
        )}
      </div>
