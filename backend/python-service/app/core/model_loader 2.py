...
from transformers import (
    AutoConfig, AutoProcessor, AutoModelForSpeechSeq2Seq,
...
class StandardModelLoader(BaseModelLoader):
    ...
    async def load_model(self) -> LoadingResult:
        ...
        try:
            ...
            # Load processor with Voxtral-specific handling
            try:
                ...
                if self.model_info.model_type == ModelType.VOXTRAL:
                    self._add_warning(f"Voxtral processor failed (missing mistral-common): {processor_error}")
                    self._add_warning("Attempting Voxtral fallback to Whisper for speech recognition")
                    
                    # Fallback to Whisper for speech recognition functionality
                    fallback_model = "openai/whisper-small"
                    logger.info(f"🔄 Voxtral fallback: Loading {fallback_model} for speech functionality")
                    ...
                    # Update model name for further processing
                    self.model_info.model_name = fallback_model
                    self.model_info.model_type = ModelType.WHISPER
